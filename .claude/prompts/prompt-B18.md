...
**파싱에 성공한 값은 지금으로부터 얼마나 떨어져 있든 정본이다.**
지연 도착을 이유로 수신 시각으로 바꾸지 마라 — 그러면 장애 복구로 뒤늦게 온
과거 문의가 최신으로 올라가 목록 미리보기를 덮는다.

파싱에 **실패한** 값(형식 불일치, 존재하지 않는 날짜)은 조용히 대체하지 말고
결정적 독약으로 격리해라. 판단 기준은 "얼마나 오래됐나"가 아니라
**"이 문자열이 날짜인가"**다.
...
### 독약 격리 — 무엇이 독약인지가 핵심이다

영구 불량 항목이 무한 재전송을 유발하면 안 된다. `mo_failures` 테이블이 이미
있다. 3회 실패하면 격리하고 `10000`을 반환해 나머지 트래픽이 흐르게 해라.

**단, 실패를 두 종류로 갈라야 한다.**

| 종류 | 예 | 처리 |
|---|---|---|
| **결정적** — 페이로드 탓 | 필수 필드 누락, 날짜 파싱 실패, 형식 위반 | 횟수에 **포함**. 3회면 격리 |
| **일시적** — 인프라 탓 | D1 혼잡·내부 오류, batch 실패, 타임아웃 | 횟수에 **넣지 마라**. 항상 재전송 코드 |

**섞으면 고객 메시지를 영구히 잃는다.** D1이 잠깐 세 번 흔들리면 멀쩡한 문의가
독약으로 오판되고, `10000`을 받은 LGU+는 재전송을 멈춘다. 그 메시지는 DB에
한 번도 들어오지 못한 채 사라진다.

**구조로 가르는 게 가장 확실하다** — 순수한 검증 단계를 먼저 두고, 거기서
걸린 것만 결정적 실패로 센다. 검증을 통과한 뒤 쓰기에서 난 오류는 전부
일시적으로 취급해 재전송시켜라.

### `moKey` 없는 항목도 격리할 수 있어야 한다

`moKey`를 못 얻으면 격리 키가 없어 3회 격리에 진입하지 못하고, 그 요청은
영원히 실패한다. **같은 묶음의 정상 메시지까지 무기한 인박스에 안 보인다.**

원본 항목의 **안정적인 해시**를 격리 식별자로 써라. 실제 `moKey`와 섞이지
않게 접두어를 붙여라.
...
8. **결정적** 실패 3회 → `mo_failures` 기록 + `10000`
8b. **일시적**(D1 오류) 실패는 몇 번이든 격리되지 않고 재전송 코드가 나간다.
    실제로 D1을 세 번 실패시켰다가 복구시켜 확인해라
8c. `moKey` 없는 항목이 같은 묶음의 정상 항목을 영구히 막지 않는다
9. 잘못된 시크릿 → 404
10. `moRecvDt` 파싱이 KST 기준이다 (경계를 테스트로 명시)
10b. **이틀 전 `moRecvDt`가 그대로 보존된다** — 수신 시각으로 바뀌지 않고,
     최신 포인터를 빼앗지도 않는다
